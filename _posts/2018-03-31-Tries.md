...
A Trie is a static tree-like data structure in which each node has 26 children pointers representing the letters of the entire English alphabet. The strings or words can be re**trie**ved by traversing down a branch path of the tree[^1]. 

{% highlight C Language linenos %}
typedef struct trieNode
{
    int count; // when traversing down a path, the count will indicate if the string is a word and the strings frequency. 
...
```
To start off, the root node of a trie is empty with 26 links to other nodes - one for each possible alphabetical value and so on.
